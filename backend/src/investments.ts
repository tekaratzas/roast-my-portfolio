  ...
  public async getHoldings(publicToken: string): Promise<InvestmentsResponse> {
    ...
    const accessToken = await this.authenticationController
      ...
      .catch((error) => {
        ...
        throw new Error(
          "Error getting access token: " + error.response.data.error_message
        );
      });
    ...
    const response = await this.plaidClient
      ...
      .catch((error) => {
        ...
        throw new Error(
          "Error getting holdings: " + error.response.data.error_message
        );
      });
    console.log(await this.getPolygonSecurity("SPY"));
